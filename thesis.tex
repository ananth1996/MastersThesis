...
\newcommand{\FTITLE}{Äänestysverkkojen päätelmät online-vaaleissa}
\newcommand{\STITLE}{Avsluta omröstningsnätverk i onlineval}
\newcommand{\SUBTITLE}{}
\newcommand{\FSUBTITLE}{}
\newcommand{\SSUBTITLE}{}
...
\newcommand{\FINSTRUCTOR}{Diplomi-insinööri Blank}
\newcommand{\SINSTRUCTOR}{Diplomingenjör Blank}
...
As votes tend to be for or against a particular agenda, they can be intuitively be represented by positive and negative links respectively in a \textit{signed network}.
...
In this thesis, we propose two new approaches for solving the task of vote prediction in signed networks.
The first is a \textit{graph combination method} that gathers features from multiple auxiliary graphs and features encoding balance and status theories.
Then, it is a supervised machine learning problem and can be trained using a general linear model.
Second, we propose a novel \textit{iterative method} to learn relationships between users to predict votes.
We quantify a network's adherence to status theory using the concept of \textit{agony} and hierarchies in directed networks.
Analogously, we measure the balance in a network through the spectral decomposition of the network.
Then, we use these measures to predict the votes that comply the most with the social theories.

We implement our approaches to predict votes in the elections of administrators in Wikipedia.
Our experiments and results on the \wikirfa dataset show that the iterative models perform much better the graph combination model.
We analyse the impact of the voting order on the performance of the models.
...
Firstly, I would like to extend my deepest gratitude to Professor Aristides Gionis and Dr. Bruno Ordozgoiti, for their constant support and guidance. Their wisdom and help is what made this thesis possible.

I would like to thank all the members of the Data Mining Group for the many deep and insightful discussions. The passion, curiosity and motivation I enjoyed as a part of the group has encouraged me to pursue a career in research.
...
\input{3vote-prediction.tex}

\input{4wikipedia.tex}
